 ...
 */
function createHealthCheckServer(port: number, mcpServer: MCPServer): HttpServer {
 ...
 */
function handleHealthCheck(res: any, mcpServer: MCPServer): void {
  try {
    const health = {
      status: 'healthy',
 ...
 */
function handleStatusCheck(res: any, mcpServer: MCPServer): void {
  try {
    const status = {
      server: {
 ...
 */
function handleMetrics(res: any, mcpServer: MCPServer): void {
  try {
    const metrics = {
      memory: process.memoryUsage(),
...
async function main(): Promise<void> {
  try {
    ...
    logger.setLevel(config.server.logLevel as any);
