...
function loadSchemas() {
  try {
    ...
    const designTokenSchema = JSON.parse(
      readFileSync(join(process.cwd(), 'src/schemas/design-token.schema.json'), 'utf-8')
    );
    const componentSchema = JSON.parse(
      readFileSync(join(process.cwd(), 'src/schemas/component.schema.json'), 'utf-8')
    );
    const guidelineSchema = JSON.parse(
      readFileSync(join(process.cwd(), 'src/schemas/guideline.schema.json'), 'utf-8')
    );
 ...
 */
export function validateDesignTokenData(data: unknown): ValidationResult {
  const valid = validateDesignToken(data);
  return {
    valid,
    errors: valid ? undefined : validateDesignToken.errors?.map(err => 
 ...
 */
export function validateComponentData(data: unknown): ValidationResult {
  const valid = validateComponent(data);
  return {
    valid,
    errors: valid ? undefined : validateComponent.errors?.map(err => 
      `${err.instancePath} ${err.message}`
    )
  };
}
 ...
 */
export function validateGuidelineData(data: unknown): ValidationResult {
  const valid = validateGuideline(data);
  return {
    valid,
    errors: valid ? undefined : validateGuideline.errors?.map(err => 
      `${err.instancePath} ${err.message}`
    )
  };
}
 ...
 */
export function validateDesignTokens(tokens: unknown[]): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateGuidelines(guidelines: unknown[]): ValidationResult {
  const errors: string[] = [];
